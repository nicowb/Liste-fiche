...
const colorIndex= {
  ...
  '-0.7':'#ffb3b3','-0.8':'#ffb3b3','-0.9':'#ffb3b3','-1.0':'#ffb3b3','-1.1':'#ff9999','-1.2':'#ff9999','-1.3':'#ff9999',
  '-1.4':'#ff9999','-1.5':'#ff9999','-1.6':'#ff8080','-1.7':'#ff8080','-1.8':'#ff8080','-1.9':'#ff8080','-2.0':'#ff8080',
            ...
            <List.Item key={item.id}>
              ...
              <span className={classes.rowC}>
                <PhfPaper //props pour index
                  ...
                  />
                {item.val<0?(
                  <Button style={{backgroundColor:getColor(item.val), marginLeft:0,marginTop:2,width:50,textAlign:'right',fontSize:14,height:30}}>{item.val}</Button>
                  ):
                  (<Button style={{marginLeft:0,marginTop:2,width:50,textAlign:'right',fontSize:14,height:30}}>{item.val}</Button>
                  )
                }
                <Slider range
                  tipFormatter={format?formatter:null}
                  marks={getValeur(item.val,echelle)} 
                  value={tableau(item.val)} 
                />
              </span>
            </List.Item>
          )}
